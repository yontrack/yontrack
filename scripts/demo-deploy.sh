...
# Build lookup, shared with scripts/demo-smoke.sh so that the workflow that deploys a version
# and the workflow that validates it resolve the same build.
...
dd_resolve_named() {
    yontrack_build_by_display_name "$DD_PROJECT" "$DD_BRANCH" "$1"
}
